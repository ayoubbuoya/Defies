...
contract LiquidityManagerCallbackTest is Test {
    ...
    function testMintCallbackFlow() public {
        ...
        assertEq(
            balanceBefore0 - balanceAfter0,
            amount0Used,
            "Token0 balance should decrease by amount used"
        );
        assertEq(
            balanceBefore1 - balanceAfter1,
            amount1Used,
            "Token1 balance should decrease by amount used"
        );

        // Verify pool received tokens
        assertEq(
            token0.balanceOf(address(mockPool)),
            amount0Used,
            "Pool should receive token0"
        );
        assertEq(
            token1.balanceOf(address(mockPool)),
            amount1Used,
            "Pool should receive token1"
        );
    ...
    function testCallbackDataDecoding() public {
        ...
        LiquidityManager.MintCallbackData memory originalData = LiquidityManager
            .MintCallbackData({
        ...
        LiquidityManager.MintCallbackData memory decoded = abi.decode(
            encoded,
            (LiquidityManager.MintCallbackData)
        );
        ...
        assertEq(
            decoded.amount0Max,
            originalData.amount0Max,
            "Amount0Max should match"
        );
        assertEq(
            decoded.amount1Max,
            originalData.amount1Max,
            "Amount1Max should match"
        );
    }
}
...
contract MaliciousContract {
    ...
    function callMintCallback() external {
        LiquidityManager.MintCallbackData memory fakeData = LiquidityManager
            .MintCallbackData({
                token0: address(0x1),
                token1: address(0x2),
                payer: msg.sender,
        ...
        liquidityManager.uniswapV3MintCallback(
            1 ether,
            1 ether,
            abi.encode(fakeData)
        );
